...
export default class ZTweener {
    ...
    private EvaluateAndApply() {
        ...
        if (this.startValue instanceof Vector3) {
            ...
            //mgLog.log(`[ZTweener:EvaluateAndApply][${this.activeId}] ${position}, ${this.duration} ${newValue.ToString()}`);
        }
            ...
            //mgLog.log(`[ZTweener:EvaluateAndApply][${this.activeId}] ${position}, ${this.duration} ${newValue}`);
        }
        this.setter(newValue);
    }

    /// custom add by metalgeni
    public SetEndVal<T>( endval : T )
    {        
