...
import { Datas, ERROR, Tags } from '../Managers/TypeManager';
...
export default class ZepetoPlayersManager extends ZepetoScriptBehaviour {
    ...
    private set currentPlayers(value: Map<string, Player>) { this._currentPlayers = value; }
    ...
    public SetRoom(room: Room) {
        if(!room || !room.IsConnected) return console.error(ERROR.ROOM_DISCONNECTION);
        this._room = room;
    }
    ...
    private CreateSinglePlayer() {
        ...
        if(!this._room) this.SetRoom(GameManager.instance.room);
    ...
    private OnJoinPlayer(sessionId: string, player: Player) {
        ...
        if(this.ZepetoPlayerSpawnType == ZepetoPlayerSpawnType.MultiplayerSpawnOnJoinRoom) {
            ...
            // ZepetoPlayers.instance.CreatePlayerWithZepetoId(sessionId, NPC_ID.YUBIN, spawnInfo, isLocal);
        }
    }
    ...
    private AddPlayerSync(sessionId:string){
        if(!this._room) {
            this.SetRoom(GameManager.instance.room);
            this._room.OnStateChange += this.OnStateChange;
        }
        ...
        zepetoPlayer.character.transform.gameObject.tag = Tags.Player;
